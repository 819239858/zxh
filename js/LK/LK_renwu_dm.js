...
function confirm() {
    ...
    layer.confirm('' + confirm + '', {
        ...
        var pointType = localStorage.getItem("pointTpye");
        var task_obj = $('#people').val();
        if (to_name != '' && to_name != null && task_region != '' && task_end_time != '') {
            sendAjax({
                "url": "fire/task/addTask",
                "data": {
                    "task_type": task_type, "task_end_time": task_end_time,
                    "task_region": task_region, "task_latlng": task_latlng,
                    "to_name": to_name, "task_title": task_title,
                    "pointType": pointType, "task_images": task_result_image,
                    "task_obj": task_obj, 'task_desc': desc
                }
...
function toCenter(task_id, task_status) {
    sendAjax({
        ...
        "data": { "task_id": task_id }, "callback": function (data) {
                        ...
                        layer.open({
                            ...
                            btn1: function () {
                                ...
                                sendAjax({
                                    ...
                                    "data": {
                                        "task_id": task_id, "task_result_image": task_result_image,
                            ...
                            area: ['400px', '400px;'],
                            title: "反馈任务",
                            content: "<div style=\"margin:30px 0 0 30px;\"><table><tr style=\"display:block;\"><span>完成内容</span><div style=\"display:inline-block;vertical-align:middle;width:80%\">" +
                                "<input type='radio' name='payMethod' value='0' checked />林火<input type='radio' name='payMethod' value='1'/>草原火<input type='radio' name='payMethod' value='2'/>计划烧除<input type='radio' name='payMethod' value='3'/>农用火<input type='radio' name='payMethod' value='4'/>炼山<input type='radio' name='payMethod' value='5'/>灌木火<input type='radio' name='payMethod' value='6'/>工矿用火<input type='radio' name='payMethod' value='7'/>其他<input type='radio' name='payMethod' value='8'/>境外火<input type='radio' name='payMethod' value='9'/>未找到<input type='radio' name='payMethod' value='10'/>核查中</div></td></tr><tr style=\"display:block;margin-top:20px;\"><td><span>任务图片</span><div style=\"margin-left:3px;width:60px;height:60px;border-radius:3px;border:1px dashed #dddee1;position: relative;display:inline-block;vertical-align: middle;text-align:center;\"><input id='imgel' style='width:60px;height:60px;left:0px;top:0px;position:absolute;opacity:0;z-index:1330;' type='file'><img style=\"margin-top:20px\"src='img/LK/tjtp.png'></div><ul id=\"lookimg\"style=\"display:inline-block;\"></ul></td></tr></table></div>",
                        })
                        $("#imgel").takungaeImgup({
                            formData: {
                                "path": "task_image",
            ...
            //弹窗
            if (task_status != "-2") {
                //如果是已经拒绝的任务就显示拒绝理由  0  result.task_status
                //if(task_status =='0' && result.task_status=='-3'){
                if (result.task_status == '-3') {
                    layer.open({
                        type: 1,
                        skin: 'layui-layer-molv', //样式类名
                        closeBtn: 1, //关闭按钮
...
function zdzx() {
    ...
    task_type == '' ? task_type = '1,2,3,4,5,6,7' : '';
    sendAjax({
        "url": "fire/task/getTaskByCondition",
        "data": {
            "per_page": 20, "current_page": 1, "task_status": task_status, "task_region": region,
            "task_type": task_type
        },
        "callback": function (data) {
    ...
    $(".pagination").createPage({
        ...
        backFn: function (p) {
            sendAjax({
                "url": "fire/task/getTaskByCondition",
                "data": { "per_page": 20, "current_page": p, "task_status": task_status, "task_region": region, "task_type": task_type }, "callback": function (data) {
                    var data = data.var.data;
                    $("#dmrw_tbody").html("");
                    for (var i = 0; i < data.length; i++) {
                        $("#dmrw_tbody").append("<tr><td>" + data[i].task_id + "</td><td>" + data[i].region_name + "</td><td>" + rewutype.get(data[i].task_type) + "</td><td>" + data[i].add_name + "</td><td>" + data[i].recv_name + "</td><td>" + data[i].task_add_time + "</td><td>" + data[i].task_end_time + "</td><td class='taskStatus_overdue'>" + rewuStatus.get(data[i].task_status) + "</td><td><a onclick='toCenter(" + data[i].task_id + "," + data[i].task_status + ");' style='color:#01dacf' href='javascript:'>" + '详情' + "</a></td></tr>");
