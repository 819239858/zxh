      ...
      "callback":function(data){
        ...
        if (data.code=="s_ok") {
          ...
          for(var i=0;i<data.length;i++){
              if(userLevel!='1'){
                  $("#dmrw_tbody").append("<tr class=[cen'><td>"+data[i].task_id+"</td><td>"+data[i].region_name+"</td><td>"+rewutype.get(data[i].task_type)+"</td><td>"+data[i].add_name+"</td><td>"+data[i].recv_name+"</td><td>"+data[i].task_add_time+"</td><td>"+data[i].task_end_time+"</td><td class='taskStatus_overdue'>"+rewuStatus.get(data[i].task_status)+"</td><td><a onclick='toCenter(" + data[i].task_id + ","+data[i].task_status+");' style='color:#addc9d' href='javascript:'>"+'详情'+"</a><a onclick='toRemove("+data[i].task_status+","+data[i].task_id+");'  style='margin-left:10px;color:#addc9d'>取消</a></td></tr>");
              }else{
