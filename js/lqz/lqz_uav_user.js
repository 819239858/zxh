...
function refresh1() {
    ...
    sendAjax({
        ...
        "data":{"current_page": 1, "region":city,"name": user_name, "mid":4,"per_page":20,"examine":1 },
            ...
            if (result.code!="s_ok") {
                ...
                $(".complete_page").createPage({
                    ...
                    backFn: function(p) {
                        sendAjax({
                            "url":"fire/user/getUserList",
                            "data":{"current_page":p,"region":city,"name":user_name,"mid":4,"per_page":20,"examine":1},
                            "callback":function(result){
                                var result = result.var.data;
                                $("#complete_report").html('');
                ...
                for (var i = 0; i < result.length; i++) {
                    $("#complete_report").append("<tr><td>" + result[i].name + "</td><td>" + result[i].tel + "</td><td>" + result[i].region_name + "</td><td>" + result[i].job + "</td><td>" + userLevel.get(result[i].level) + "</td><td>" + result[i].role + "</td><td><a class=\"mr-5\" onclick=\"report_index('" + result[i].uid + "')\">详情</a></td><td><label class=\"radio\" onclick=\" nodisable('" + result[i].uid + "')\"><input type=\"radio\" name=\"" + result[i].tel + "\" class=\"search\"/><span>是</span></label><label class=\"radio\" onclick=\" disable('" + result[i].uid + "')\"><input type=\"radio\" name=\"" + result[i].tel + "\" class=\"screening\"/><span>否</span></label></td>" +
                        "<td><a class=\"mr-5\" onclick=\"reset_index('" + result[i].uid + "')\">重置密码</a></td></tr>");
                    if (result[i].status=="0") {
                        $("#complete_report").find(".screening").eq(i).prop("checked", true);
                    }else{
...
function news_user(e) {
    ...
    layer.confirm('' + news_user + '', {
        ...
        if (name!=''&& tel!= '' && pwd != '' && pwd1 != ''&& job != '' ) {
            sendAjax({
                ...
                "data":{"tel":tel,"password":pwd,"region":region,"name":name,"mids":sum2,"imgHead":imgHead,"job":job,"rid":rid},"callback":function(data){
    ...
    $(".comm_add").click(function (){
        n++;n<10?$(".add_comm_add").after('<li><label>飞机型号</label><input type=\"text\" class=\"form-control form-boxed planeNo\" value=\"\"><label>序列号</label><input type=\"text\" class=\"form-control form-boxed serialNum\" value=\"\"><label>是否起用</label><label class=\"radio\"><input type=\"radio\" name=\"isFly1\" class=\"screening\" value=\"1\"/ checked><span>是</span></label><label class=\"radio\"><input type=\"radio\" name=\"isFly1\" class=\"search\" value=\"0\"/><span>否</span></label><label>头像</label><input type=\"file\" id=\"photoName2\" class=\"upFileBtn\" accept=\"image/png,image/jpg,image/gif,image/JPEG\"> <img class=\"fileInput\" src=\"img/lqz/upimg.png\"><div id=\"imgBox1\"></div></li>'):layer.alert("不能超过10个", {skin: 'layui-layer-molv' ,closeBtn: 0,anim: 4,btnAlign: 'c'});
    });
}
// 无人机用户详情
...
function disable(result) {
    layer.confirm("禁用后该账户无法使用，是否确认？", {
        ...
        sendAjax({
            ...
            "data":{"uids": [result], "mid":4,"status":0 },"callback":function(result){
...
function nodisable(result) {
    layer.confirm("启用后该账户正常使用，是否确认？", {
        ...
        sendAjax({
            "url":"fire/user/editUserStatus",
            "data":{"uids": [result], "mid":4,"status":1 },"callback":function(result){
...
function information(e) {
    ...
    sendAjax({
        "url":"fire/user/getUserList",
        "data":{"current_page": 1, "region":city,"name": user_name, "mid":4,"per_page":20,"examine":0 },
        "callback":function(result){
            var num = '';
            if (result.code!="s_ok") {
                ...
                $(".complete_page1").createPage({
                    ...
                    backFn: function(p) {
                        sendAjax({
                            "url":"fire/user/getUserList",
                            "data":{"current_page":p,"region":city,"name":user_name,"mid":4,"per_page":20,"examine":0},
                            "callback":function(result){
                                var result = result.var.data;
                                $("#complete_report2").html('');
                                for (var i = 0; i < result.length; i++) {
...
function audit() {
    ...
    layer.confirm("已经提交无人机用户注册审核，是否通过?", {
        btn: ['同意', '拒绝'],
        skin: 'layui-layer-molv',
        title: '提示',
        btnAlign: 'c'
        ...
        sendAjax({
            ...
            "data":{"uids": name_array, "mid":4,"examine":1 },
            "callback":function(result){
                if (result.code == "s_ok") {
                    layer.msg("审核通过！");
...
$("#report_btn").click(function() {
    if ($(this).text() == "无人机审核信息") {
        ...
        $(this).html("<i class=\"icon-file-alt\"></i>无人机审核信息");
    }
});