...
function news_user() {
    var add_resources ="<ul class=\"form_sub form_sub_release\" style=\"width:750px\"><li><label><i style='color:red'>*</i>区域：</label>" +
        "<select id=\"province\" class=\"remove_disabled\" onchange=\"callback(value,this)\"></select><select id=\"city_two\" class=\"remove_disabled\" onchange=\"callback(value,this)\"></select><select id=\"village\" class=\"remove_disabled\"></select><label><i style='color:red'>*</i>新闻类型：</label><select id='newstype' class='remove_disabled'><option value='0'>防火动态</option><option value='1'>当前火情</option><option value='2'>火险预报</option><option value='3'>经验交流</option></select></li><li><label><i style='color:red'>*</i>新闻标题：</label><input type=\"text\" class=\"form-control form-boxed\" value='' id=\"usertitle\"></li><li><label><i style='color:red'>*</i>作者：</label><input type=\"text\" class=\"form-control form-boxed\" value=\""+sessionStorage.getItem('name')+"\" id=\"author\"></li>" +
    ...
    layer.confirm(add_resources, {
        ...
        if (author!=''&&content!=''&&title!=''&&region!=''&&newsType!='') {
            sendAjax({
                "url":"fire/news/saveNews",
                "data":{"region": region,"publishName":publishName,"author":author,"title":title,"content":content,"attchment":attchment,"news_type":newsType,"images":images},
...
function information(e) {
    ...
    var data_info=data_info2=data_info3=data_info4={"per_page": 20,"current_page":1,
        "news_type": newsType, "region": region,"start_time": startTime,"end_time":endTime};
...
function list_one(data_info) {
    sendAjax({
        ...
        "data":data_info,"callback":function(result){
            if(result.code='s_ok'){
                //总条数
                var num = '';
                if (result.var.total == 0) {
                    $(".complete_total1").html(0);
                    num = 1;$("#complete_report1").html('');
                } else {
...
function audit() {
    ...
    layer.confirm(audit, {
        ...
        sendAjax({
            ...
            "data":{"id": audit_name,"rebut_reason":rebutReason,"status":"1"},"callback":function(result){
                if (result.code == "s_ok") {
                    layer.msg("审核通过！");
        ...
        sendAjax({
            "url":"fire/news/examineNews",
            "data":{"id": audit_name,"rebut_reason":rebut_reason,"status":"-1"},"callback":function(result){
                if(rebutReason==''){layer.msg("请填写驳回原因！");return false;}
...
function assigned(){
    ...
    layer.confirm(assigned, {
        ...
        sendAjax({
            ...
            "data":{"id": audit_name,"region":region,"to_name":userphone},"callback":function(result){
                if (result.code == "s_ok") {
