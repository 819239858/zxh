...
function refresh1() {
    ...
    sendAjax({
        ...
        "data":{"current_page": 1, "region":city,"name": user_name, "mid":2,"per_page":20,"examine":1 },
            ...
            if (result.code!="s_ok") {
                ...
                $(".complete_page").createPage({
                    ...
                    backFn: function(p) {
                        sendAjax({
                            "url":"fire/user/getUserList",
                            "data":{"current_page":p,"region":city,"name":user_name,"mid":2,"per_page":20,"examine":1},
                            "callback":function(result){
                                var result = result.var.data;
                                $("#complete_report").html('');
                                for (var i = 0; i < result.length; i++) {

                                    $("#complete_report").append("<tr><td>" + result[i].name + "</td><td class=\"lt\">" + result[i].tel + "</td><td>" + result[i].region_name + "</td><td>" + result[i].job + "</td><td>" + userLevel.get(result[i].level) + "</td><td>" + result[i].role + "</td><td><a class=\"mr-5\" onclick=\"report_index('" + result[i].uid + "')\">详情</a></td>" +
                                        "<td><label class=\"radio\" onclick=\" nodisable('" + result[i].uid + "')\"><input type=\"radio\" name=\"" + result[i].tel + "\" class=\"search\"/><span>是</span></label><label class=\"radio\" onclick=\" disable('" + result[i].uid + "')\"><input type=\"radio\" name=\"" + result[i].tel + "\" class=\"screening\"/><span>否</span></label></td>" +
...
function news_user(e) {
    ...
    layer.confirm('' + news_user + '', {
        ...
        var job=$("#job").val();
        if (name!=''&& tel!= '' && pwd != '' && pwd1 != ''&& job != ''&&position!='' ) {
            sendAjax({
    ...
    sendAjax({
        ...
        "data":{"uid":fireId},"callback":function(result){
            ...
            if (result.code == "s_ok") {
                ...
                if(sessionStorage.getItem("userLevel")=='3'||sessionStorage.getItem("userLevel")=='2'){
                    ...
                    if(result.imgHeadUrl.length==0){
                        layer.open({
                            ...
                            title: "护林员用户详情",
                            shadeClose: true, //开启遮罩关闭
                            content: "<ul class=\"form_sub form_sub_release\" style=\"width:568px\"><li><label>头像</label><input type=\"file\" id=\"photoName1\" class=\"upFileBtn\" accept=\"image/png,image/jpg,image/gif,image/JPEG\" style=\"display:none\"> <img class=\"fileInput\" src=\"img/lqz/upimg.png\" style=\"display:none\"><div id=\"imgBox\"><img class=\"imgbig\" src="+Public_address+'uploads/'+result.imgHead+" onerror=\"javascript:this.src='img/timg.png'\"></div></li><li><label>画区域<i style='color:red'>*</i></label><input type=\"text\" class=\"form-control form-boxed\" disabled id=\"latLngs\" style=\"margin-right:2px;width:135px;\" value=\""+result.position+"\"><input class=\"enter_map_two\" type=\"button\" style=\"display:none\" value=\"进入地图\" onclick=\"sessionStorage.setItem('hidebtn','1');layer.open({type: 2,title: '选中地图',shadeClose: false,shade: 0.8,area: ['700px', '450px;'],content: 'map.html',cancel: function(){$('#latLngs').val(localStorage.getItem('site'));sessionStorage.removeItem('hidebtn');}})\" /><input class=\"enter_map\" type=\"button\" value=\"管护区域\" onclick=\"echo_map()\"></li><li><label>用户名<i style='color:red'>*</i></label><input type=\"text\" class=\"form-control form-boxed remove_disabled\" value=\"" + result.name + "\" id=\"username\" disabled><label>手机<i style='color:red'>*</i></label><input type=\"text\" class=\"form-control form-boxed\" value=\"" + result.tel + "\" id=\"userphone\" disabled></li><li id=\"password\"><label>密码<i style='color:red'>*</i></label><input type=\"password\" class=\"form-control form-boxed remove_disabled\" value=\"" + sessionStorage.getItem("paw") + "\" id=\"userpwd\" disabled><i class=\"icon-lock\"></i><label style=\"width:82px;\">确认密码<i style='color:red'>*</i></label><input type=\"password\" class=\"form-control form-boxed remove_disabled\" value=\"" + sessionStorage.getItem("paw") + "\" disabled id=\"userpwd1\"><i class=\"icon-lock\"></i></li><li><label>管辖区域<i style='color:red'>*</i></label><input type=\"text\" class=\"form-control form-boxed\" value=\"" + result.region_name + "\"  disabled><label>职务<i style='color:red'>*</i></label><input type=\"text\" class=\"form-control form-boxed remove_disabled\" value=\"" + result.job + "\" id=\"job\" disabled></li><li><label>类型<i style='color:red'>*</i></label><div id=\"qx_yh\" style=\"background-color:#f2f2f2\"><input type=\"checkbox\">后台用户<input type=\"checkbox\">护林员<input type=\"checkbox\">护林员<input type=\"checkbox\">无人机<input type=\"checkbox\">载人机</div></li><li><label>角色</label><select id=\"character\" style=\"width:150px;background-color:#f2f2f2\" class=\"\" disabled><option value='1'>一般用户</option><option value='2'>管理员</option><option>超级管理员</option></select><label>注册时间</label><input type=\"text \" id=\"creactTime\" disabled onclick=\"WdatePicker({dateFmt:yyyy-MM-dd})\" class=\"Wdate form-control\" value=\"" + result.create_time + "\"></li></li>" +
                            // "<li class=\"background_user_information\"><input type=\"button\" value=\"编辑\" class=\"report\" onclick=\"remove_disabled()\"> " +   下面保存按钮带的属性  ：onclick=\"disabled_user('" + result.uid + "')\"  disabled
                            "<li class=\"background_user_information\"><input type=\"button\"  value=\"保存\" class=\"report submit\" onclick=\"clickMod()\"></li></ul>"
                        });
                    }else{
                        layer.open({
                            type: 1,
                ...
                sum2=[];
                for (var index = 0; index < result.mold.length; index++) {
                    sum2.push(result.mold[index].mid);
                    var element = result.mold[index].mid-1;
...
function disable(result) {
    layer.confirm("禁用后该账户无法使用，是否确认？", {
        ...
        sendAjax({
            ...
            "data":{"uids": [result], "mid":2,"status":0 },"callback":function(result){
...
function nodisable(result) {
    layer.confirm("启用后该账户正常使用，是否确认？", {
        ...
        sendAjax({
            "url":"fire/user/editUserStatus",
            "data":{"uids": [result], "mid":2,"status":1 },"callback":function(result){
...
function information(e) {
    ...
    sendAjax({
        "url":"fire/user/getUserList",
        "data":{"current_page": 1, "region":city,"name": user_name, "mid":2,"per_page":20,"examine":0 },"callback":function(result){
            ...
            if (result.code!="s_ok") {
                ...
                $(".complete_page1").createPage({
                    ...
                    backFn: function(p) {
                        sendAjax({
                            "url":"fire/user/getUserList",
                            "data":{"current_page":p,"region":city,"name":user_name,"mid":2,"per_page":20,"examine":0},"callback":function(result){
    ...
    sendAjax({
        "url":"fire/user/getUserList",
        "data":{"current_page": 1, "region":city,"name": user_name, "mid":2,"per_page":20,"examine":1 },"callback":function(result){
...
function audit() {
    ...
    layer.confirm("已经提交护林员用户注册审核，是否通过?", {
        btn: ['同意', '拒绝'],
        skin: 'layui-layer-molv',
        title: '提示',
        btnAlign: 'c'
        ...
        sendAjax({
            ...
            "data":{"uids": name_array, "mid":2,"examine":1 },"callback":function(result){
...
$("#report_btn").click(function() {
    if ($(this).text() == "护林员审核信息") {
        ...
        $(this).html("<i class=\"icon-file-alt\"></i>护林员审核信息");
    }
});
// 回显地图
function echo_map() {
