	...
	layer.confirm(''+add_resources+'', {
		...
		if (region!=''&&location!=''&&established_time!=''&&long!=''&&width!=''&&administrator!=''&&phone!='') {
			sendAjax({
				...
				"data":{"region":region,"location":location,"established_time":established_time,
					"length":long,"width":width,"describe":describe,"team_image":imgName,
...
function query() {
	...
	sendAjax({
		"url":"fire/fire_barrier/getFireBarrierList",
						...
						backFn:function(p){
							...
							"callback":function(data){
								...
								for(var i=0;i<result.length;i++){
                                    if(userLevel!='1'){
                                        $("#complete_report").append("<tr><td>" + result[i].region_name + "</td><td>" + result[i].location + "</td>><td>" + result[i].width + "</td><td>" + result[i].length + "</td><td>" + result[i].established_time + "</td><td>" + result[i].input_time + "</td><td><a class='mr-5'>详情</a> <a onclick=\"delete_data(" + result[i].id + ")\">删除</a></td></tr>");
                                    }else{
                                        $("#complete_report").append("<tr><td>" + result[i].region_name + "</td><td>" + result[i].location + "</td>><td>" + result[i].width + "</td><td>" + result[i].length + "</td><td>" + result[i].established_time + "</td><td>" + result[i].input_time + "</td><td><a class='mr-5'>详情</a></td></tr>");
                                    }
								}
...
function delete_data(fireid) {
	layer.confirm('确定要删除吗？',{
		...
		sendAjax({
		"url":"fire/fire_barrier/delFireBarrier",
