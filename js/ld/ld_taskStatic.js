...
sendAjax({
	...
	"data":{"start_time":year+"-"+month,"end_time":year+"-"+next,"task_region":task_region,"type":$("#ld_content").val()},
...
//统计数据解析
	...
	if($("#ld_startTime").val()==''){
		...
		var title=$("#ld_city option:checked").html()+$("#ld_startTime").val().substring(0,4)+'年'+$("#ld_startTime").val().substring(5)+'月至'+$("#ld_endTime").val().substring(0,4)+'年'+$("#ld_endTime").val().substring(5)+'月'+countContent+"统计图"
	}
	var option = {
		title: {
				text:title,
		...
		tooltip: {
			...
			formatter: function (params) {
				...
				if($('#ld_content').val()==""){
					...
					'月总计'+params.data+'条</p>'+'<p>热点核查:<span style="color:#fff;">'+item.total5+'</span>(完成率:'+(item.count5).toFixed(2)+'%'+')</p>'+
					'<p>日常核查:<span style="color:#fff;">'+item.total6+'</span>(完成率:'+(item.count6).toFixed(2)+'%'+')</p>'+
					'<p>紧急核查:<span style="color:#fff;">'+item.total7+'</span>(完成率:'+(item.count7).toFixed(2)+'%'+')</p>'+
					'<p>森林火灾调查:<span style="color:#fff;">'+item.total8+'</span>(完成率:'+(item.count8).toFixed(2)+'%'+')</p>'+
					'<p>林业有害生物:<span style="color:#fff;">'+item.total9+'</span>(完成率:'+(item.count9).toFixed(2)+'%'+')</p>'+
					'<p>破坏森林资源调查:<span style="color:#fff;">'+item.total10+'</span>(完成率:'+(item.count10).toFixed(2)+'%'+')</p>'+
					'<p>偷猎野生动物调查:<span style="color:#fff;">'+item.total11+'</span>(完成率:'+(item.count11).toFixed(2)+'%'+')</p>'
					/*+'<p>空中喷洒:<span style="color:#fff;">'+item.total12+'</span>(完成率:'+(item.count12).toFixed(2)+'%'+')</p>'*/
					;
				return res;
				}else{
					var res='<div><p>'+params.name.substring(0,4)+'年'+params.name.substring(5)+
					'月总计'+params.data+'条</p>'+
					'<p>消防员:<span style="color:#fff;">'+item.total0+'</span>(完成率:'+item.count0+'%'+')</p>'+
					'<p>护林员:<span style="color:#fff;">'+item.total1+'</span>(完成率:'+item.count1+'%'+')</p>'+
					'<p>无人机:<span style="color:#fff;">'+item.total2+'</span>(完成率:'+item.count2+'%'+')</p>'+
					'<p>载人机:<span style="color:#fff;">'+item.total3+'</span>(完成率:'+item.count3+'%'+')</p>'
					;
				return res;
				}
			}
...
//横轴
   ...
   $("#ld_search").click(function(){
	...
	sendAjax({
		"url":"fire/task/countTaskData",
		"data":{"start_time":$("#ld_startTime").val(),"end_time":$("#ld_endTime").val(),"task_region":$("#ld_city").val(),"type":$("#ld_content").val()},
		"callback":function(data){
			if (data.code=="s_ok") {
